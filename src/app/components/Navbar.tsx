...
export function Navbar() {
  ...
  return (
    <>
      <nav
        className={`fixed left-1/2 -translate-x-1/2 z-50 transition-all duration-500 rounded-full border flex items-center overflow-hidden ${
          scrolled
            ? "top-4 w-[92%] max-w-5xl border-white/15 shadow-[0_12px_40px_rgba(0,0,0,0.6)] py-2.5 px-8"
            : "top-6 w-[95%] max-w-6xl border-white/5 py-3.5 px-8"
        }`}
      >
        {/* Dynamic Image Background with Blur & Opacity */}
        <div
        ...
        <div
          className={`absolute inset-0 z-0 pointer-events-none transition-all duration-500 ${
            scrolled ? "bg-[#0e100f]/75 backdrop-blur-md" : "bg-[#0e100f]/35 backdrop-blur-sm"
          }`}
        />
        ...
        <div className="w-full flex items-center justify-between relative z-10">
            ...
            {navLinks.map((link) =>
              link.highlight ? (
                <li key={link.label}>
                  <motion.a
                    href={link.href}
                    className="px-5 py-2 bg-[#c62828] text-white rounded-full tracking-widest uppercase text-xs block font-semibold transition-all duration-300"
          ...
          {/* Mobile toggle */}
          <motion.button
            className="md:hidden text-[#c62828] z-50 relative p-2"
            onClick={() => setOpen(!open)}
            aria-label="Toggle menu"
            whileHover={{ scale: 1.1, rotate: open ? 90 : 0 }}
            whileTap={{ scale: 0.9 }}
          >
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className="fixed inset-0 z-40 bg-[#0e100f] flex flex-col justify-between px-8 py-24 md:hidden h-screen w-screen overflow-hidden"
          >
            {/* Background design accents */}
            <div className="absolute inset-0 opacity-[0.02] pointer-events-none select-none flex items-center justify-center">
              <span className="text-[35vw] font-bold text-[#c62828]" style={{ fontFamily: "'Noto Sans JP', sans-serif" }}>
                祭
              </span>
            </div>
            
            {/* Top decorative line */}
            <div className="absolute top-24 left-8 right-8 h-px bg-white/5" />

            {/* Menu Links */}
            <div className="flex flex-col gap-8 my-auto pl-4">
                ...
                >
                  ...
                  >
                    <div className="flex items-baseline gap-4">
                      <span className="text-stone-600 text-xs font-light">0{idx + 1}</span>
                      <span
                        className={`text-4xl uppercase tracking-wider font-light transition-all duration-300 ${
                          link.highlight
                            ? "text-[#c62828] group-hover:text-white"
                            : "text-stone-200 group-hover:text-[#c62828] group-hover:pl-2"
                        }`}
                    ...
                    {/* Subtext Japanese translations for Japanese aesthetics */}
                    <span 
                      className="text-stone-500 text-[10px] tracking-widest uppercase pl-8 mt-1 group-hover:text-stone-400 transition-colors duration-300"
                      style={{ fontFamily: "'Noto Sans JP', sans-serif" }}
                    >
                      {link.label === "Home" && "ホーム | Main"}
            ...
            {/* Footer accents within mobile overlay */}
            <motion.div
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
              exit={{ opacity: 0, y: 10 }}
              transition={{ delay: 0.4, duration: 0.5 }}
              className="flex justify-between items-center text-[10px] text-stone-500 tracking-widest border-t border-white/5 pt-6"
              style={{ fontFamily: "'Noto Sans JP', sans-serif" }}
            >
              <span>IIT DHARWAD</span>
              <span>HARSHTAL © 2026</span>
            </motion.div>
          </motion.div>
        )}
      </AnimatePresence>
